...
export function TestMapScreen() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const nick = `V${Math.random().toString(36).slice(2, 5)}`;
        room = await new Client(SERVER_URL).joinOrCreate("baseworld", { nickname: nick });
        if (!alive) { void room.leave(); return; }
        startGame(room, hostRef.current ?? undefined, undefined, { noSprites: true });
      } catch (e) {
        if (alive) setError(e instanceof Error ? e.message : "접속 실패");
      }
    })();
    return () => { alive = false; stopGame(); void room?.leave(); };
  }, []);

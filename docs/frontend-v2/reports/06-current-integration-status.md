...
- Socket.IO phase advances now persist the same `phaseEndsAt` into the backend API room snapshot, so REST refetch/reconnect observes the same timer metadata.

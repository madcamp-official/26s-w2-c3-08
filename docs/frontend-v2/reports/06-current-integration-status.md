...
- Production env readiness now fails closed on missing `backend.NODE_ENV=production` or `gpu-worker.NODE_ENV=production`, missing/placeholder credentials, missing backend `INTERNAL_API_TOKEN`, localhost public URLs, backend/gpu-worker worker token mismatch, backend/gpu-worker image storage mode mismatch, invalid image storage mode, and `GPU_WORKER_SIMULATE=true`. GPU worker startup also fails closed on unsafe production worker tokens, simulate mode, inline storage, placeholder Qwen/WAN credentials, and invalid storage URLs when `NODE_ENV=production`.

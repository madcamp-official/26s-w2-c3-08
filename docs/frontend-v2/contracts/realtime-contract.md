...
| `game.raceFinished` | client emits `race:finish`; server emits `race:finished`; REST uses `/api/rooms/:roomId/race/finish` | `{ roomId, userId, finishTimeMs }` | client/backend | room clients | backend keeps minimum finish time | if disconnected after finish, results final should include finish | idempotent by min finish time | `backend race:finish`, REST `race/finish`, backend HTTP contract test, `finishGameRace` |

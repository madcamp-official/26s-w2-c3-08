...
| `room.timeVoteRequested` | `time_vote:request` | `{ roomId, userId, phase, deltaSec }` | client | backend realtime | backend room timer; future vote aggregation `TBD-CONTRACT` | user may retry once per phase; reconnect can refetch REST room snapshot with applied `phaseEndsAt` | idempotency key missing; use `(roomId,phase,userId,deltaSec)` | `requestRealtimeTimeVote`, `backend time_vote:request`, `adjustApiRoomPhaseEndsAtFromRealtime` |
| `room.timeVoteUpdated` | `time_vote:updated` | `{ roomId, phase, deltaSec, voterIds?, approved?, applied?, remainingMs?, phaseEndsAt? }` | backend realtime | client store | backend room timer; future vote aggregation `TBD-CONTRACT` | latest update wins; REST refetch observes the applied timer value | idempotent by phase and `phaseEndsAt`; voter set pending | `realtime.ts RealtimeTimeVoteUpdatedPayload`, backend emits applied timer delta and persists it to API room snapshot |
...
| Semantic event name | Current transport event | Payload | Sender | Receiver | Authoritative source | Reconnect behavior | Idempotency | Source evidence |
|---|---|---|---|---|---|---|---|---|
| `game.racePositionSent` | `race:position` | `{ roomId, userId, x, y, vx, vy, state, progress, clientTime }` | racing client | backend realtime | client-authoritative position in MVP, with monotonic best progress persisted to API race snapshot | remote positions marked stale until next event; reconnect can refetch best progress from REST room/race result | idempotent by max progress per `userId` | `RaceCanvas onProgress`, `sendRealtimeRacePosition`, `backend race:position`, `setApiRoomRaceProgressFromRealtime` |
| `game.racePositionReceived` | `race:position` | same as above | backend broadcast | other clients | emitting client through backend plus API progress snapshot | stale if no update; refetch result/snapshot for durable progress | idempotent by max progress per `userId` | `backend socket.to(...).emit`, `realtime.ts onRacePosition`, backend socket contract test |
| `game.raceFinished` | client emits `race:finish`; server emits `race:finished`; REST uses `/api/rooms/:roomId/race/finish` | `{ roomId, userId, finishTimeMs }` | client/backend | room clients | backend keeps minimum finish time in API race result and socket room state | if disconnected after finish, results final and REST result include the canonical minimum finish time | idempotent by min finish time | `backend race:finish`, `finishApiRoomRaceFromRealtime`, REST `race/finish`, backend HTTP/socket contract tests, `finishGameRace` |
...
| RT-GAP-004 | RESOLVED for V2 remote MVP: backend uses full `room:state` for presence/ready sync, and `room:ready`/`phase:ready` persist readiness into the API room snapshot. Fine-grained local-only lobby events are legacy/dev conveniences. |

...
| DECISION-V2-005 | APPROVED | 일반 에셋 생성 후 Asset Studio에 머문다. 성공 toast와 "창고에서 진행 상황 보기" CTA를 표시한다. canvas/form은 유지하고 새 에셋 만들기에서만 초기화한다. | 현재 `appStore.submitAsset`은 non-avatar 제출 후 `view: 'warehouse'`, `warehouseTab: 'component'`로 이동한다. | V2 store/controller에서 non-avatar success navigation 변경 필요. |
...
| DECISION-V2-007 | APPROVED | 사용자 제작 컴포넌트 카테고리는 `platform`, `obstacle`, `monster`, `background`다. `item`은 V2 MVP에서 system-provided asset이다. | `AssetStudio`의 `studioCategories`는 item/avatar 제외. `Warehouse` component filter도 item 제외. | `MapBuildPhase` shelf는 system item을 배치 대상으로 포함하므로 user-created와 system shelf 구분 필요. |
...
| DECISION-V2-009 | APPROVED | Asset job 상태는 push 우선이고 asset job에 한해 제한적 polling fallback을 허용한다. | `realtime.ts`는 `asset_job:updated` handler가 있고, `appStore.tickMockGeneration`은 API source에서 pending asset이 있을 때 5초 refresh polling을 한다. | `backend/src/socket/index.ts`는 현재 asset job push emit이 없다. job id 기반 polling endpoint도 현재 backend route에는 없다. |
| DECISION-V2-010 | APPROVED | V2 MVP remote room/game realtime은 `backend/` Socket.IO 계약을 우선한다. `server/` Colyseus는 대체 transport 후보로만 문서화한다. UI/feature 계층은 transport에 의존하지 않는다. | `backend/src/socket/index.ts`는 Socket.IO room/game events 구현. `client/src/net/realtime.ts`는 현재 Colyseus SDK + BroadcastChannel. | frontend realtime adapter를 Socket.IO 우선으로 교체하거나 bridge가 필요하다. |
| DECISION-V2-011 | APPROVED | `VITE_DATA_MODE=mock|remote`를 사용한다. remote 실패 시 Mock으로 자동 fallback하지 않는다. | 현재 `api.ts`는 `VITE_REMOTE_API === 'true'`이고 fetch 실패 시 `null`을 반환해 mock으로 내려간다. | env 이름과 fallback 정책 변경 필요. |
| DECISION-V2-012 | APPROVED | `VITE_REALTIME_MODE=local|remote`를 사용한다. remote 실패 시 BroadcastChannel로 자동 fallback하지 않는다. | 현재 `realtime.ts`는 `VITE_LOCAL_REALTIME !== 'false'`이면 BroadcastChannel fallback을 연다. | env 이름과 fallback 정책 변경 필요. |
| DECISION-V2-013 | APPROVED | Figma Variables와 CSS tokens는 동일 token source를 사용한다. 권장 원본 파일은 `design/tokens.json`이다. | `FINAL_PLAN.md`는 `tokens.css` 계획만 포함한다. | `design/tokens.json`은 현재 없다. token generation gate 필요. |
...
| DECISION-V2-015 | APPROVED | Playwright E2E와 screenshot 기반 시각 검증은 V2 완료 게이트다. | Phase 0 baseline에는 client lint/smoke/build만 있다. | Playwright/screenshot 검증이 아직 없다. |
...
| TBD-CONTRACT-PD-002 | Asset job push event `asset_job:updated`를 backend Socket.IO worker가 언제/어떤 payload로 emit할지 결정 필요. | TBD-CONTRACT |
...
| TBD-CONTRACT-PD-005 | Playwright screenshot baseline 저장 위치와 viewport matrix 결정 필요. | TBD-CONTRACT |

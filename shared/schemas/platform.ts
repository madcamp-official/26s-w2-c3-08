...
import { z } from "zod";
import { FacesAttr, Period3, Power2, SizeCells, Speed3 } from "./presets.js";
...
const Collision = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("faces"), faces: FacesAttr }), // 특정 면만 = 일방통행
]);
...
const ContactReaction = z.discriminatedUnion("type", [
  ...
  z.object({ type: z.literal("fall"), senseFaces: FacesAttr }),                 // n초 후 낙하 → 재생
  z.object({ type: z.literal("break"), senseFaces: FacesAttr }),                // n초 후 파괴 → 재생
]);
...
/** 충돌 방식 → 4면 플래그 통일 (md §1). 반환값은 물리 엔진의 Faces(terrain.ts)와 구조가 같아 그대로 대입 가능 */
